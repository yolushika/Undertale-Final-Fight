...
/**
 * Act select enemy state - select which enemy to act upon
 */
public class ActSelectEnemyState extends AbstractMenuState {
    
    @Override
    public MenuStateType getStateType() {
        return MenuStateType.ACT_SELECT_ENEMY;
    }
    
    @Override
    public void handleSelect(MenuStateContext context) {
        context.getSoundManager().playSE("confirm");
        context.setState(StateFactory.createState(MenuStateType.ACT_SELECT_ACT));
    }
